...
int main() {
    ...
    csv_file << "矩阵大小,原始算法(ms),行优化算法(ms),NEON/向量化(ms),预取优化(ms),NEON加速比,预取加速比,NEON标准差,预取标准差\n";
    
    // 设置固定的重复次数
    const int REPEAT_TIMES_LARGE = 5;  // 大矩阵重复次数
    const int REPEAT_TIMES_SMALL = 20; // 小矩阵重复次数
    ...
    for (int n = 1; n <= 2000;) {
        ...
        // 根据矩阵大小动态调整步长和重复次数
        if (n <= 100) {
            repeat_times = REPEAT_TIMES_SMALL;
            n += 5;  // 小矩阵使用较小步长
        } else if (n <= 500) {
            repeat_times = REPEAT_TIMES_SMALL;
            n += 25; // 中等矩阵使用中等步长
        } else {
            repeat_times = REPEAT_TIMES_LARGE;
            n += 100; // 大矩阵使用较大步长
        }
